	...
	<li>
		...
		<span class="player-stat-value">
			...
			<small class="player-stat-small"
				>#{formatNumber(playerCategory.rank, ",")} ({country}#{formatNumber(playerCategory.countryRank, ",")})</small>
		</span>
	</li>
	...
	{#if playerCategory.gainedScores != null}
		<li>
			...
			<span class="player-stat-value">
				...
				<small class="player-stat-small"
					>since {formatDate(addDays(new Date(playerCategory.date), -((playerCategory.gainedDays ?? 0) + 1)))}</small>
			</span>
		</li>
	{/if}
</ul>
