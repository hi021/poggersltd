...
<dialog
	...
	style="--index: {verticalIndex.current};">
	<h3>
		<span>
			{player.name}
			<small style="font-weight: 400;" use:tooltip={{ content: `${category}s count on ${player.stats.endDate}` }}
				>({formatNumber(player.stats.endScores)})</small>
		</span>
		<form method="dialog">
			<button type="button" aria-label="Close" class="btn-icon" onclick={close}><icon class="close"></icon></button>
		</form>
	</h3>
	...
	<ul class="player-stats-container ul">
		...
		<li>
			...
			<span class="player-stat-value">
				...
				<small>
					({Math.round(((player.stats?.endScores - player.stats?.startScores) / days) * 100) / 100}/day)
				</small>
			</span>
		</li>
		<li>
