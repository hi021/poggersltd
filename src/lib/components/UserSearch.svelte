<script lang="ts">
	...
	function handleClick(e: MouseEvent) {
		...
		if (!target?.className?.includes?.("autocmp-item") && !target?.className?.includes?.("search-input"))
			autocompleteEntries = [];
	}
			...
			{#each autocompleteEntries as a (a._id)}
				<li transition:slide={{ duration: 100, axis: "y" }}>
					...
					<a
						...
						<img class="osu-avatar-small" alt="" src={getAvatarURL(a._id)} style="margin-right: 1rem;" />
						{a.name}
					</a>
				</li>
			{/each}
