...
function calculatePlayerStats(ranks: Array<App.ComparisonChartEntryWithDate | null>): App.PlayerProfileStatsWithDates {
...
export const GET: RequestHandler = async ({ params, setHeaders }) => {
	...
	const dateStart = isNaN(dateStartTimestamp as number) ? Date.parse(MIN_DATE) : (dateStartTimestamp as number);
	const dateEnd = isNaN(dateEndTimestamp as number) ? getServerDate().valueOf() : (dateEndTimestamp as number);
	...
	for (const tmpId of idsOrNames) {
		allPlayerPromises.push(...prepareFetchPlayer(scoreCategory, tmpId, days, datesToFetch, players));
	}

	await Promise.all(allPlayerPromises);
	for (const i in players) {
		players[i].ranks = players[i].ranks.filter(a => a).sort((a, b) => ((a?.date ?? "0") < (b?.date ?? "0") ? 1 : -1));
