...
export const GET: RequestHandler = async ({ params, setHeaders }) => {
	...
	if (gainedDays === 1) {
		const rankingDataSorted = rankingDataEnd.filter(plr => plr.gainedScores != null).sort(sorting);
	...
	for (const entry of rankingDataStart) players.set(entry._id, { scores: entry.scores, rank: entry.rank });
