...
export function _processResult(resPlayersJson: App.ComparisonChartAPI, setColorsAndVisibility = false) {
...
export function _processPlayerRanksObject(ranksObj: ComparisonChartEntriesObject) {
	...
	for (const timestamp in ranksObj) ranksArray[i++] = { date: timestamp, players: ranksObj[timestamp] };
...
export function _mergePlayerRanksIntoExistingArray(
	...
	return _processPlayerRanksObject(mergeObjectArraysOnField(ranksSrc, ranksDest, "date", "players"));
}
...
export function _pruneExistingDataOutsideDateRange(
	...
	const startDate = range.start ? new Date(range.start).valueOf() : "";
	const endDate = range.end ? new Date(range.end).valueOf() : "Z";
	return { players, ranks: ranks.filter(it => it.date >= startDate && it.date <= endDate) };
}
...
export const load: PageLoad = async ({
	...
	const resPlayers = await fetch(`/api/players/${params.idsOrNames}${categoryString}${dateRangeString}`);
