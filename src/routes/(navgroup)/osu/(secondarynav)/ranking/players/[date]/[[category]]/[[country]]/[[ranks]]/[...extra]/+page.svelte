<script lang="ts">
	...
	let pageData = $derived(
		data.rankingData.slice($rankingSettings.perPage * (curPage - 1), $rankingSettings.perPage * curPage)
	);
...
<main class="flex-fill column osu-main">
	...
	<RankingSettings viewMode="players" />
	{#if !pageData?.length}
		<RankingEmpty />
	{:else}
				...
				{#each pageData as plr, i (plr._id)}
					<tr class:top-rank={plr.rank <= 5} id="rank-{plr.rank}">
						...
						<td style="width: 25%;">
							...
							{plr.gainedScores == null ? "" : `(${(plr.gainedScores ?? -1) >= 0 ? "+" : ""}${plr.gainedScores})`}
						</td>
					</tr>
