...
<main class="flex-fill column" style="padding: 0 3.5%">
	<form class="row" spellcheck="false" onsubmit={preventDefault(() => gotoPlayerOnEnter({ name: username }))}>
		<UserSearch disabled={true} {gotoPlayer} {gotoPlayerOnEnter} bind:value={username} />

		<select class="input-dark" bind:value={mode} use:tooltip={{ content: "Game mode" }} disabled={true}>
	...
	<div class="row" style="align-items: center; margin-top: 16px; width: fit-content;">
		<Switch disabled={true} bind:checked={singleRank} style="flex-direction: row; gap: 8px; align-items: center;">
			{#snippet after()}
