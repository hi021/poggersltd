	...
	{:else}
		<div class="chart-container" style="height: {chartHeight}px;">
			{#each data.breakdown as d, i}
				<div
					...
					style="width: calc({100 / data.breakdown.length}% - 4px); left: {(i / data.breakdown.length) * 100}%;">
					<div
		...
		<div class="stats-container">
			...
			<small class="row" style="justify-content: center; margin-top: 12px;">
				<span>out of</span><strong style="margin: 0 1ch;">{formatNumber(data.beatmaps)}</strong><span>maps</span>
			</small>
		</div>

		<button type="button" class="btn-gray" style="margin-bottom: 16px;" onclick={() => (showRaw = !showRaw)}>
			Show raw data
		</button>
		{#if showRaw}
