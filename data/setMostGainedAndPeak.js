...
try {
	...
	for (const file of globFiles) {
		...
		for (const category in fileJson) {
			...
			for (const plr of fileJson[category]) {
				...
				if (!plrFromDatabase) {
					console.warn(`${plr.name} (${_id}) is in the ranking entry file but not in the database!`);
					continue;
				}
				...
				if (plrFromDatabaseCategory.peak?.value == null || plrFromDatabaseCategory.peak.value < plr.scores) {
					plrFromDatabaseCategory.peak = o;
					change = true;
				}
				if (plrFromDatabaseCategory.lowest?.value == null || plrFromDatabaseCategory.lowest.value > plr.scores) {
	...
	for (const i in playersNew) {
		promises[i] = new Promise(resolve =>
			dbPlayers.updateOne({ _id: playersNew[i]._id }, { $set: playersNew[i] }).then(() => resolve(1))
		);
	}
