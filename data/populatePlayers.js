...
function putPlayerIntoMap(players, player, category, date) {
	if (players.has(player._id)) {
		...
		if (
			...
			player.gainedScores &&
			(!playerObject[category].mostGained || playerObject[category].mostGained.scores < player.gainedScores)
		) {
		...
		else if (player.gainedScores) playerObject[category].mostGained = { date, scores: player.gainedScores };
