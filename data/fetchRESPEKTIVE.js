...
import { formatDate, getRankingCollections, getClosestPrevArchiveEntry, createNGram } from "./shared.js";
...
async function fetchCategory(category, minScores, maxPage = MAX_PAGE) {
	...
	while (page <= maxPage) {
		try {
			...
			console.error(`Failed to fetch ${category} (page ${page}, lastMinScores ${lastMinScores}):`, e);
			return null;
		}
	}
...
for (const i in categories) {
	...
	try {
		...
		fs.writeFileSync(path.join(__dirname, "archive", date, cat + ".json"), JSON.stringify(categoryWithGains));
		...
		console.log(`Most gained ${cat} was ${mostGainedCategory.gained} by ${mostGainedCategory.name}`);
		...
		if (lastArchive.daysLate) {
			console.log("Gains are " + lastArchive.daysLate + " day(s) late, skipping mostGained ranking");
		} else {
			...
			if (mostGainedCategory.gained > mostGainedLowest) {
				for (const plr of categoryWithGains) if (plr.gained >= 4) mostGained[cat].push({ ...plr, date });
				...
				console.log(`Lower than lowest ranked player in mostGained ranking (${mostGainedLowest}), skipping`);
			}
		}

		const insertRes = await coll.insertOne({
			_id: cat,
			ranking: categoryWithGains
		});
			...
			new Promise(async resolve => {
				...
				if (playerFromDatabase && playerFromDatabase.name !== playerCurrent.name) {
					...
					if (playerFromDatabase.oldName)
						playerCurrent.oldName = [...new Set([...playerFromDatabase.oldName, playerFromDatabase.name])];
					...
					for (const i of collections) {
						...
						if (findRes?.length) {
							for (const i in findRes) {
								...
								await coll.updateOne({ _id: findRes[i]._id }, { $set: { ranking: findRes[i].ranking } });
							}
						}
					}
				...
				for (const i of categories) {
					...
					if (lastArchive?.daysLate === 0 && playerCurrent[cat].gained != null) {
						if (
							...
							playerCurrent[cat].mostGained = {
								date,
								value: playerCurrent[cat].gained
							};
					}
					...
					//set from database if no change
					if (playerCurrent[cat].peak?.value == null) playerCurrent[cat].peak = playerFromDatabase?.[cat]?.peak;
					if (playerCurrent[cat].lowest?.value == null) playerCurrent[cat].lowest = playerFromDatabase?.[cat]?.lowest;
				}

				const updateRes = await collPlayers.updateOne({ _id }, { $set: playerCurrent }, { upsert: true });
