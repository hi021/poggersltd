...
try {
	...
	const playersDatabase = await client.db(process.env.DB_NAME).collection("players").find().toArray();
	...
	for (const file of globFiles) {
		...
		const insertRes = await coll.updateOne({ _id: date }, { $set: convertedFile }, { upsert: true });
