...
for (const category in startData) {
	...
	for (const player of startData[category]) {
		...
		arr[category][player._id].avg = Math.round((arr[category][player._id].gainedScores / daysBetween) * 100) / 100;
	}
}

for (const cat in arr) for (const plrId in arr[cat]) arrFinal[cat].push({ ...arr[cat][plrId], id: plrId });
for (const cat in arrFinal) arrFinal[cat].sort((a, b) => (a.gainedScores < b.gainedScores ? 1 : -1));
